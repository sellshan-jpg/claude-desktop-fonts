...
- **注入内容仅有字体规则。** 写入页面的只有 CSS `@font-face` 声明，作用范围限于
  ...
  或计费。
- **不接触账号与会话数据。** 不读取、不上传账号信息与聊天记录，不与本项目之外的
  任何服务器通信。
- **可完整撤销。** 安装前创建完整应用备份；任一环节失败或中途取消都会自动回滚并
  重新验证签名。「还原」在能匹配到当前版本备份时，会连同 Anthropic 的原始签名一并
  恢复。
...
Xcode 命令行工具为必需依赖。重新签名与失败回滚均依赖其提供的 `codesign`。
Clfont 会在修改任何文件之前检测该组件，若不可用则直接终止操作。
...
3. **兼容模式**：保持「标准」即可；若个别区域未生效，再切换至「扩展」，该模式会
   将同样的覆盖规则扩展到更多常见字体族。
4. **执行安装**：点击「应用」，整个过程约需 1 至 2 分钟。

安装流程依次为：退出目标应用 → 创建完整备份 → 重新打包 `app.asar` → 更新完整性
哈希 → 重新签名 → 启动验证。任一环节失败或用户中途取消，均会自动回滚至原始状态
并重新签名验证。
