...
final class Copy: ObservableObject {
    ...
    static let zh: [String: String] = [
        ...
        "footer.safety": "Clfont 仅在本机修改 Claude 的显示样式：注入内容只有字体与底色的样式规则，不改动任何网络请求，也不读取账号信息与聊天记录。",
        "header.subtitle": "在 Claude 中使用你偏好的字体与配色",
        ...
        "target.hint": "建议先在测试 Claude 上确认效果，再应用到正式 Claude。",
        "stale.action": "重新应用",
        ...
        "appmgmt.recheck": "已开启，重新检测",
        "appmgmt.title": "无法修改 Claude：缺少「App 管理」权限",
        "update.auto": "启动时自动检查更新",
        "update.body": "下载后替换「应用程序」中的 Clfont 即可，所有设置均会保留。若此前已为 Claude 应用过，更新后请重新执行一次「应用」。",
        ...
        "look.group": "外观",
        "look.bg": "页面底色",
        "look.bg.desc": "将 Claude 的界面底色恢复为早期版本的米色。仅在浅色模式生效。",
        "look.bg.off": "不修改",
        "look.bg.custom": "自定",
        "look.bg.classic": "经典",
        "look.bg.light": "浅米",
        ...
        "busy.install": "应用到「{target}」：备份 → 重打包 → 重签名 → 启动验证（约 1–2 分钟）",
        "busy.uninstall": "还原「{target}」：恢复原文件 → 重签名",
        "busy.doctor": "自检「{target}」",
        "busy.prune": "清理旧备份",
        "busy.create": "创建测试副本：复制 → 更换独立标识 → 重签名 → 迁移登录状态",
        "busy.remove": "移除测试 Claude：删除应用副本、配置目录与对应备份",
        ...
        "sheet.restore.title": "确认还原 Claude？",
        "sheet.restore.body": "将从备份恢复{target}的 app.asar 并重新签名，当前的字体、代码块与底色设置均会被移除。Claude 需要退出后重新打开。",
        ...
        "update.norepo": "尚未发布到 GitHub，暂无可检查的版本。",
        "update.found": "有新版本 {tag}。",
        "update.latest": "已是最新版本。",
        "update.norelease": "仓库尚未发布任何版本。",
        "update.failed": "检查失败：{why}",
        "update.err.norepo": "仓库地址没配置",
        "update.err.rate": "GitHub 请求频率超限，请稍后重试",
        ...
        "about.tagline": "替换 Claude 桌面版的显示字体，并可调整代码块字体与页面底色",
        "about.dev": "开发者",
    ...
    static let en: [String: String] = [
        ...
        "footer.safety": "Clfont changes how Claude looks on this Mac and nothing more. It injects style rules for fonts and the background only: no network request is touched, and your account and conversations are never read.",
        "header.subtitle": "Your preferred fonts and colours, inside Claude",
        "target.hint": "Try the settings on the test Claude before touching the real one.",
        ...
        "update.body": "Download it and replace Clfont in your Applications folder. All settings carry over. If you have already applied changes to Claude, apply once more afterwards.",
        ...
        "font.latin.where.desc": "Replies only changes Claude's replies and headings; Everything also takes the interface, the composer and the messages you send",
        ...
        "look.bg.desc": "Restores the cream background of earlier Claude versions. Light mode only.",
        ...
        "busy.remove": "Removing the test Claude: the copy, its profile and its backups",
        ...
        "sheet.restore.title": "Restore Claude?",
        "sheet.restore.body": "This restores {target}'s app.asar from backup and re-signs it. Your font, code block and background changes are all removed. Claude quits and reopens.",
        ...
        "update.norepo": "Not published to GitHub yet, so there is nothing to check.",
        "update.found": "Version {tag} is available.",
        "update.latest": "You're on the latest version.",
        "update.norelease": "The repository has no releases yet.",
        ...
        "about.tagline": "Replaces the display fonts in Claude for desktop, with settings for code blocks and the page background",
        "about.dev": "Developer",
...
@MainActor final class Model: ObservableObject {
    ...
    /// 界面字号：侧边栏、输入框、你自己发的消息。与正文分开，因为侧边栏基准
    /// 只有 13px，跟着正文一起放大时肉眼看不出变化。
    ...
    func saveConfig() {
        ...
        j["font_scale_ui"] = Int(fontScaleUI.rounded())
...
struct ContentView: View {
    ...
    private func statusHeadline(_ s: AppStatus) -> String {
        ...
        // 只报字体名会说不准：可能只改了底色或代码块字体。字体名放到副标题里。
        if s.patched { return t("status.applied") }
    ...
    private func statusSubline(_ s: AppStatus) -> String {
        ...
        if !s.font.isEmpty { parts.insert(s.font, at: 1) }
