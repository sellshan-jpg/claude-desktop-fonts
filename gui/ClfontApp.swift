...
final class Copy: ObservableObject {
    ...
    static let zh: [String: String] = [
        ...
        "help.step2.body": "应用字体需要给 Claude 重新签名，原始签名会因此被替换。为避免反复改动日常使用的应用，建议先在测试 Claude 上确认效果。\n测试 Claude 是正式版的完整副本，拥有独立的应用标识与配置目录，对它的任何操作都不影响正式版。应用到测试 Claude 时不弹确认，可以随意尝试。",
        ...
        "help.step7.body": "将 Claude 的界面底色换成早期版本的米色。提供「经典」「浅米」「米灰」三档预设，也可用取色器自选。\n仅在浅色模式生效。深色模式下该设置不起作用，无需担心配色被改坏。",
    ...
    static let en: [String: String] = [
        ...
        "help.step2.body": "Applying fonts means re-signing Claude, which replaces its original signature. Rather than keep reworking the app you use all day, try each font on the test Claude first.\nThe test Claude is a full copy with its own app identity and its own profile, so nothing you do to it reaches the real one. Applying to it skips the confirmation, so experiment freely.",
...
let releaseNotes: [ReleaseNote] = [
    ReleaseNote(
        ...
        zh: [
            ...
            "新增「页面底色」，可将 Claude 的界面底色换成早期版本的米色。提供三档预设，也可自行取色。仅在浅色模式生效。位于「外观」一栏。",
            "新增「代码块」设置，可单独指定代码块与行内代码的等宽字体与字号，正文不受影响。",
            "新增「界面」字号，单独调整侧栏、底栏、输入框与用户发送消息的字号。这些位置本身只有 12–13px，跟随正文缩放时变化不明显，因此拆开。",
            "新增「替换范围」，换英文时可只改 Claude 的回复与标题，或连界面一起改。小号界面标签换成正文衬线体后会偏细，此选项用于避免该问题。",
            ...
            "应用流程新增确认与进度提示：应用到正式版前先行确认，过程中显示实时日志，完成后说明钥匙串授权并自动启动 Claude。",
            "设置项按「中文 / 英文 / 界面」分组，使用指引重写。",
        ],
        en: [
            "The Xcode Command Line Tools are no longer required. The command-line half was rewritten and now ships inside the app.",
            "Added a page background, which brings back the cream tone of earlier Claude versions. Three presets, or pick your own colour. Light mode only. Under Appearance.",
            ...
            "Added an interface size for the sidebar, the bottom bar, the composer and the messages you send. These sit at 12–13px, where scaling with the body goes unnoticed, so they now have their own dial.",
            ...
            "The interface comes in Simplified Chinese and English, following your system by default. It sits behind the globe icon in the title bar, and only changes Clfont, never Claude.",
            "The apply flow now confirms before touching your main Claude, shows a live log while it works, explains the keychain prompt, and starts Claude for you afterwards.",
            "Settings are grouped by Chinese, English and Interface, and the guide has been rewritten.",
        ],
